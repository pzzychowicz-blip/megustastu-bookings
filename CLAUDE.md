...
- **`Overlay` is UNCHANGED — Fix 4 was deliberately NOT applied.** `overflow` clips at the *padding box* and the desktop sheet already has 24px padding, giving scale-only lifts ~24px of room (empirically: full-width input 19px margin, footer Save 21px). The doc's inner-scroller (`overflow:visible` + `padding:"4px 16px"`) would *reduce* that to 4px and clip the footer — counterproductive here. Don't add it without re-checking this. (Footer-anchoring, a separate UX nicety, is therefore also not done — open for a future pass.)
