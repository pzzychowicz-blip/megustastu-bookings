...
│   └── useLayout.js                 editable table layout (tables add/remove/rename · capacity · zone · join-groups · combos) → constants.js live bindings via setLayout; sanitize enforces single-group membership; Firebase settings/layout (v15.0.0; 4th settings node)
├── components/
...
│   ├── LayoutSettings.jsx           Layout-tab body (v15.0.0) — FULL layout editor: Tables (add/remove/rename·cap·zone, orphan-booking warning on remove/rename) + collapsible Combos (editable join-groups → auto-combo cap overrides + cross-group mega add/edit/remove) + kitchen limit; takes `bookings` for orphan detection
...
└── lib/
    ├── booking-logic.js             pure functions (optimizer, sanitisation, derivations, daySummary); v15.0.0: isIn via ZONE_OF, optimizer heuristics gated by IS_MGT_LAYOUT, date-finders read hoursFor(date)
    ├── constants.js                 layout config — DEFAULT_LAYOUT + setLayout/buildLayout reassign LIVE bindings (ALL_TABLES/INDOOR/OUTDOOR/TIMELINE_TABLES/TOTAL_SEATS/ZONE_OF/TABLE_GROUPS/VALID_COMBOS/CLUSTERS/KITCHEN_TABLE_LIMIT/IS_MGT_LAYOUT) + per-weekday hours (WEEK_HOURS/hoursFor/weekRange); colours, S/BTN style tokens (v15.0.0)
