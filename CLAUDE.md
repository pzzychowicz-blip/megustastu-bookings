...
`notifAnnounce` is a sibling *after* `</main>` rather than inside it. Modals
render inline as siblings of `<main>`, so `inert` goes on **three** siblings
(header, date-nav, main); a wrapper div would re-parent the `shellFixed` flex
column. `anyModal` is ONE derivation in `App` (it was the same 17-term expression
twice in `useKeyboardShortcuts`, and `inert` would have made it three).
