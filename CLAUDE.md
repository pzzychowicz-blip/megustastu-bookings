...
│   ├── TimeAxis.jsx                 v17.5.0 — the Plan view's scrollable 15-min time-block strip (replaced the `<input type=range>` scrubber). Spans OPEN…**GRID_CLOSE**, i.e. the SAME range TimelineView draws — which is exactly why it reuses `pct()`/`QUARTER_HOURS` unchanged with no range-parametrised variant, and why a block lines up tick-for-tick with the timeline. Fixed 44px/quarter (the app's tap-target floor), no zoom — Plan already owns a pinch-zoom on the floor SVG. Alternating hour bands + hour labels (a flat row of hairline cells is invisible in dark mode); re-centres on `autoScrollKey` (bumped ONLY at programmatic scrub sites) via useLayoutEffect + INSTANT scrollLeft — `behavior:"smooth"` showed the far right then slid ~1.5s. NOT memo'd on purpose: it reads live bindings memo can't see, so gating happens in its memo'd parent via `hoursSig`
