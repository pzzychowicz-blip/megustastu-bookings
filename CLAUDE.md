...
- **No rgba/hex literals in JS — every colour references `var(--…)`.** Migrated token-by-token in waves. **v14.2.0:** core `S` set (card/border/muted/text/accent) + app background (`--bg-app`). **v14.2.1:** the `constants.js` data-token colour sets — `STATUS_COLORS` + `TBL` as **RGB-channel triplets** composed `rgba(var(--…-rgb), a)`; `BLOCK_BG` + `BTN` as direct tokens. Block/table/button tokens are **theme-invariant** (saturated fills read on both themes, defined once in `:root`); only status-chip **text** flips light in dark. **Still literal** (pending waves): `atoms.jsx` (`Overlay`, `Section`, `mkInp`, `mkBtn`, `Kbd`, `AvailBanner`) + the components — so dark mode is **partial until those land**.
...
- Shared `.mgt-hover-scale` utility: `scale(1.08)`, `120ms ease`, `border-radius:12px`, opaque theme-aware `--bg-hover-card`, the `:hover:not(:disabled)` guard. Magnitude/contract must match Scheduling exactly.
...
- **Dark mode / theming port** — mechanism shipped **v14.2.0**; **v14.2.1** migrated the `constants.js` colour sets (`STATUS_COLORS` / `BLOCK_BG` / `TBL` / `BTN`). Remaining `var(--…)` literal-migration **waves**: **14.2.2** `atoms.jsx` (`Overlay` / `Section` / `mkInp` / `mkBtn` …) → high-density components (`TimelineView`, `BookingFormModal`, `WalkinForm`) → remaining components → `App.jsx`. (Original "wave A" was split for reviewability: `constants.js` = 14.2.1, `atoms.jsx` = 14.2.2.) See `MGT_Bookings_dark-mode_PORT_INSTRUCTIONS.md`.
