...
| An effect that dispatches a NEW array on every run | `bookingsAfterAction` returns a fresh array whether or not the pass changed anything, so an effect that assigns its result unconditionally and depends on `bookings` **re-runs forever**. The v15.6.1 reconciliation effect did exactly this, and its comment claimed it was "self-stabilising: optimiser output is clean → next pass is a no-op" — which is false for the one case it cannot fix: two `_locked` bookings on one table. `applyOpt` copies a locked booking's tables through verbatim, so no reshuffle separates them, and every walk-in (`_manual:true _locked:true`) and every drag-drop path sets `_locked`. Shipped and spinning since v15.6.1; found in v17.10.2 by **reading the console**, which no amount of DOM measurement surfaces — the app looks and behaves perfectly while React burns the tablet's CPU. Its sibling branch survived only by ACCIDENT: when nothing is movable it breaks with `next` still `=== prev`, and React bails out of identical state. **Returning the original reference is now the explicit contract on both branches**, decided by `dayBookingsSig(after,d) !== dayBookingsSig(next,d)` (`booking-logic.js`, tested). Any new auto-effect that writes derived data must be able to answer "did this actually change anything" with a CONTENT compare — identity is guaranteed to differ. **And the compare must not be NARROWER than the pass it gates** (/code-review): the first version compared table assignment alone and so discarded the `duration` extension and the `_conflict` flag the very same pass writes |
...
- ~~**Tests** — no test suite~~ **STALE since v17.3.2**: a Vitest suite EXISTS — `booking-logic` · `customers` · `drafts` · `waitlist-match` · `presence-state` · `stylesheet` · `contrast` · `time-grid` · `style-check` · **`csp`**, **412 tests** as of v17.12.0 (`npm test`). CI gates build + test + **lint (0 errors, hard)** + **`npm run check:style`** on every PR via `.github/workflows/ci.yml`. No UI/component tests — UI verification is still AST audits + manual DEV QA.
