...
- **v15.5.0:** **Per-booking-node storage — structural multi-device merge.** Completes the data-integrity arc: `bookings` moves from one array node to a **keyed object `/bookings/{id}`** so two devices editing **different** bookings concurrently (incl. offline) write disjoint paths and Firebase **merges** them — eliminating the whole-array lost-write class that v15.2.0–15.4.0 could only mitigate. The global `bookingsRev` CAS is **replaced** by a per-booking `updatedAt` stamp (clock-skew- & StrictMode-proof) + a per-`$id` Security Rule; reads/UI are unchanged (still array-shaped via `sanitizeAll`), the write path diffs prev-vs-next and writes only changed children, and a lazy one-time migration converts the legacy array. **Deploy is a HARD CUTOVER** (new app vs the v15.3.0 rule are mutually incompatible). See Critical patterns → "Per-booking-node storage + diff-write" and `database.rules.README.md`. **Status: app shipped; the PROD Security Rule swap is Patryk's manual console hard-cutover step (pending).**
