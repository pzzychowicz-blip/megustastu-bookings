...
- Exceptions: the multi-export files listed in `eslint.config.js`'s `only-export-components` override.
...
4. **Save feedback + retry (v15.4.0 · v15.6.0 · v15.7.0 · v17.16.9)** — `saveBookings`/`saveBlocks` return a boolean and every handler gates its success UI on it, so a refused write is **never** shown as saved. Function-form, non-silent writes park and replay on fresh data; **value-form and silent writes never queue**, because replaying a precomputed array re-writes stale data.
...
| A new per-booking FIELD | It has to join **EIGHT** places, not the three `booking-logic.js` holds. `sanitize`, `UNDO_FIELDS` and `diffBooking` make a field survive a READ, an undo and a history entry — **none of them makes it get WRITTEN**; `doSaveNew`, `doSaveEdit`, the recurring generator and `useWalkin.js`'s walk-in build the booking field by field, and `openEdit` builds the form draft the same way. **`openEdit` is the silent one**: without it, opening and re-saving any booking WIPES the field, invisible until somebody edits one that had it. `deposit` marks seven (not the walk-in, which starts without one); `tests/booking-logic.test.js` scans `App.jsx` for the pairing — a line setting `deposit:` and `status:` must set the new field too. Found in v18.0.0 by RUNNING the app: `voucherCode` was in all three lists and every test passed |
...
| A new field on a VOUCHER | `sanitizeVoucher` is a WHITELIST, so a field missing from it is deleted by the next write to that voucher — silently, with no error anywhere (v18.0.0's `reversals`, the record a restored balance leaves, is in it for exactly that reason). **Sort the keys of any new MAP you add** (`sortedLedger` in `vouchers.js` is the existing example)**:** `write-path.js`'s `contentKey` is a key-order-sensitive `JSON.stringify` compare, so an unsorted map read back from RTDB reads as a change and writes on every pass. The test that catches the whole class is "do an unrelated write to the same voucher afterwards and assert the field is still there" |
