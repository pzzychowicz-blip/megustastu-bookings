...
├── hooks/
│   ├── usePersistence.js            Firebase + write-guards (loaded/empty + v15.2.0 freshness-resync gate) + auto-extend + auto-complete-after-close (v15.1.0)
...
- **v15.1.0: the `:hover` rule is wrapped in `@media (hover: hover) and (pointer: fine)`.** iOS Safari makes `:hover` STICKY after a tap — unguarded, the last-tapped element stayed scaled 1.08, and full-width form inputs (Date/Time in the booking form) visibly overflowed their Section on phones. Touch devices get no hover lift at all; mouse/trackpad behaviour unchanged. The guard is part of the shared contract — **port it to MGT Scheduling** (pending as of 2026-06-12).
