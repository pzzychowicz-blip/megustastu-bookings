...
- **v14.7.0:** the **Week View** (`WeekView.jsx`, an `Overlay` popover opened from the Summary panel's **Week** button) — a 7-day (Mon–Sun) at-a-glance of per-day **covers + bookings**, today/selected highlighted, ‹ › week nav; tap a day to jump to it (sets `viewDate`, closes). Counts reuse `daySummary`. Provisional **`k`** shortcut opens it (`WEEK_KEY` in `App.jsx` + the Shortcuts "K" row; `w` was taken by Walk-in — final key TBD). **Date math is all-UTC** (`new Date("YYYY-MM-DD")` + `getUTC*`/`setUTC*` + `toISOString`) to match the app's date-string convention — mixing local `getDate()` with UTC `toISOString()` shifts the week a day in UTC+ zones (caught in live QA).
