...
| A new per-booking FIELD | It has to join **SEVEN** places, not the three `booking-logic.js` holds. `sanitize`, `UNDO_FIELDS` and `diffBooking` make a field survive a READ, an undo and a history entry — **none of them makes it get WRITTEN**; `doSaveNew`, `doSaveEdit` and the recurring generator build the booking field by field, and `openEdit` builds the form draft the same way. **`openEdit` is the silent one**: without it, opening and re-saving any booking WIPES the field, invisible until somebody edits a booking that had one. `deposit` is in all seven, so grep it to find them; `tests/booking-logic.test.js` scans `App.jsx` for the pairing — any line setting `deposit:` and `status:` together must set the new field too. Found in v18.0.0 by RUNNING the app: `voucherCode` was in all three lists and every test passed |
