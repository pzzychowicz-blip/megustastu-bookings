...
│   ├── useReminders.jsx             reminder state + listeners + banner JSX — the ONLY hook that returns JSX, which is why the v14.2.x token sweep of `src/components` never saw its banner (themed at last in v17.8.0; needed new `--warn-chip-bg` + `--app-success-solid` tokens) (v16.0.0: ref-mirror saves — the set()-in-updater shape is GONE — + revGuard CAS writes; v17.8.0 owns `reminderBaseline`/`reminderDirty` for the unsaved-changes guard, diffed via a local `flatReminder()`)
