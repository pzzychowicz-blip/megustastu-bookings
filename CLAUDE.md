...
├── hooks/
│   ├── usePersistence.js            Firebase + write-guards (loaded/empty + v15.2.0 freshness-resync gate) + v15.5.0 per-booking-node diff-write + lazy array→keyed migration + auto-extend + auto-complete-after-close (v15.1.0)
...
├── components/
│   ├── BookingFormModal.jsx         booking form (controlled component; v16.0.0 phone-autocomplete dropdown + Regular/no-show customer chips + "Add to waitlist" under the no-tables banner)
│   ├── TimelineView.jsx             Gantt-style timeline (horizontal scroller; v16.0.0 start-time chip on blocks ≥140px + " ⚠" label marker for 2+ no-show phones)
...
│   ├── Settings.jsx                 settings modal shell + tabs (General/Layout/Customers/Reminders/Shortcuts — 5th tab v16.0.0); General = per-weekday hours · optimizer cutoff(0–24)/auto-switch · shifts, sections collapsible (v15.0.0)
