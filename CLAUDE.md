...
- **v15.2.0:** **Firebase stale-overwrite protection — client resync gate.** Fixes a second data-loss incident (asleep laptop overwrote a night of tablet bookings on wake). Adds a freshness dimension to the write-guard — see Critical patterns → "Firebase write-guard pattern → Freshness / resync gate". `STALE_GAP_MS=90000`; new `resyncing` banner. **Phase 2 (v15.3.0, planned, user-approved):** server-side revision backstop — a sibling `bookingsRev` integer + a Firebase Security Rule enforcing monotonic compare-and-swap on bookings writes (rejects stale writes server-side); needs a `database.rules.json` (new, version-controlled) tested on the DEV project first. Requires Patryk's current console rules to merge non-destructively.
