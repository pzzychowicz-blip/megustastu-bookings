...
- **Dark mode / theming port** — shipped: **v14.2.0** mechanism, **v14.2.1** `constants.js` colour sets, **v14.2.2** `atoms.jsx` + the full modal/form subsystem. Remaining `var(--…)` waves: **`TimelineView`** (the Gantt canvas — biggest remaining light surface) → **`ListView`** → final **`App.jsx`** main-screen banner sweep. Then the hover-scale port. See `MGT_Bookings_dark-mode_PORT_INSTRUCTIONS.md`.
