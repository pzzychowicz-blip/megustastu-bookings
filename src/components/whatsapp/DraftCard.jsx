...
export function DraftCard({ conv, onAccept, onDismiss, onDismissAcceptedBadge, compact }) {
        ...
        {hasDetail ? (
          <Reveal show={expanded} style={{ padding: "0 10px" }}>
            <div style={{ paddingBottom: 10, display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {d.ambiguity ? <div style={{ padding: "8px 10px", borderRadius: R.inset, background: "var(--danger-bg)", border: "1px solid var(--danger-border)", fontSize: T.body, color: "var(--danger-text)" }}>{"⚠ " + d.ambiguity}</div> : null}
            </div>
          </Reveal>
        ) : null}
      </div>
  ...
  return (
    <div style={{ padding: "14px 16px", borderRadius: R.card, background: "var(--wa-draft-bg)", border: "2px solid var(--wa-draft-border)", marginBottom: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
      ...
      {d.ambiguity ? (
        <div style={{ padding: "8px 10px", borderRadius: R.inset, background: "var(--danger-bg)", border: "1px solid var(--danger-border)", fontSize: T.body, color: "var(--danger-text)", marginBottom: 10 }}>{"⚠ " + d.ambiguity}</div>
      ) : null}
