...
export function AppBanners({isOnline,writeWarning,onDismissWarning,ineffShow,onDismissIneff,onReshuffle}){
  const offlineBanner=!isOnline?<div
    style={{background:"var(--app-offline-bg)",border:"2px solid var(--app-offline-border)",borderRadius:R.card,padding:"10px 14px",marginBottom:10,fontSize:13,fontWeight:700,color:"var(--app-offline-text)",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>⚠ Working offline — your changes are saved locally and will sync when the connection returns. Keep this tab open.</div>:null;
  const writeWarningBanner=writeWarning?<div
    style={{background:"var(--danger-bg)",border:"2px solid var(--danger-border)",borderRadius:R.card,padding:"10px 14px",marginBottom:10,fontSize:13,fontWeight:700,color:"var(--danger-text)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}><span>{"⚠ "+writeWarning}</span><button
  ...
  const ineffBanner=ineffShow?<div
    style={{background:"var(--warn-bg)",border:"2px solid var(--warn-border)",borderRadius:R.card,padding:"10px 14px",marginBottom:10,fontSize:13,fontWeight:600,color:"var(--warn-text)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}><span>Tables could be reshuffled for better efficiency.</span><div style={{display:"flex",gap:6}}><button
        onClick={onDismissIneff}
        className="mgt-hover-scale"
        style={mkBtn({fontSize:13,minHeight:36,padding:"6px 14px",background:BTN.dismiss})}>Dismiss</button><button
        onClick={onReshuffle}
        className="mgt-hover-scale"
        style={{background:BTN.orange,color:"var(--text-on-accent)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:R.pill,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:600,minHeight:36,boxShadow:"0 1px 4px rgba(0,0,0,0.1), inset 0 1px 1px rgba(255,255,255,0.15)"}}>Reshuffle</button></div></div>:null;
  return <>
