...
// (Running-late, Overlap warnings — WaitAvail could adopt it too). Extracted
// from the byte-identical scaffolding both had: the amber container, the
...
export function BannerRows({ title, ids, collapseMax = 2, renderRow }) {
  ...
  return (
    <div style={{ background: "var(--app-overlap-bg)", border: "2px solid var(--app-overlap-border)", borderRadius: 14, padding: "10px 14px", marginBottom: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
      <button
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--warn-text)", marginBottom: 2 }}>{title + " · " + liveCount}</span>
