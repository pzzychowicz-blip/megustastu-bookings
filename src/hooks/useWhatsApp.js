...
export function useWhatsApp({
  // v18.0.0 phase 5: `settings/admin.modules.whatsapp.enabled`, via App.
  enabled,
  bookings,
  setWriteWarning,
  ...
  // clearAllWaData.
  // The gate, read once. Every listener below depends on it, so it must be a
  // VALUE in their dep arrays — a build constant needed no dep and a runtime
  // switch does, or an admin turning WhatsApp on attaches nothing until reload.
  const on = enabled === true;
  const conversationKeysRef = useRef([]);
  const messagesMapRef = useRef({});

  // ── Listeners (read-only; never write back) ────────────────────────────────
  // PROD-leak guard (17.0.0-wa-sandbox; the gate CHANGED in v18.0.0 phase 5):
  // every listener is gated, and what it is gated ON is now the MODULE rather
  // than WA_SANDBOX. The hazard is unchanged and worth restating, because it
  // happened: a build connected to PROD Firebase attaches the templates
  // listener, which SEEDS DEFAULT_TEMPLATES into the PROD `templates` node on
  // first load — a real prod write, 2026-07-16.
  //
  // WA_SANDBOX could not survive as the gate, because it is exactly the thing
  // that made the module unreachable in production, which is what this release
  // exists to change. The module switch covers the same hazard for a stronger
  // reason: it ships OFF, so a stray preview build on the main project reads
  // `settings/admin.modules` from PROD, finds WhatsApp disabled, and attaches
  // nothing — the same outcome, now decided by the restaurant's own data
  // instead of by which env var a deployment happened to set. When an admin
  // does switch it on, seeding the templates is the intended effect of enabling
  // the module rather than a leak.
  useEffect(() => {
    if (!on) return;
