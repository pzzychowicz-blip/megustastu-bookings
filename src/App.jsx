...
var OPEN=13,CLOSE=22;
var QUARTER_HOURS=Array.from({length:(CLOSE-OPEN)*4},function(_,i){return OPEN*60+i*15;});
...
// ── Style helpers ─────────────────────────────────────────────────────────────
function mkInp(){return {width:"100%",boxSizing:"border-box",background:S.bg,border:"0.5px solid "+S.border,borderRadius:8,padding:"10px 12px",fontSize:15,color:S.text,fontWeight:500};}
...
function Overlay(props){
  return RC("div",{style:{position:"fixed",inset:0,background:"rgba(61,46,26,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:12},onClick:function(e){if(e.target===e.currentTarget)props.onClose();}},
    RC("div",{style:{background:S.card,borderRadius:12,border:"0.5px solid "+S.border,padding:"20px",width:"100%",maxWidth:580,maxHeight:"90vh",overflowY:"auto"}},props.children));
}
function Fld(props){
...
function StepperRow(props){
  var label=props.label,value=props.value,onDec=props.onDec,onInc=props.onInc,display=props.display,extra=props.extra,note=props.note;
  var btnS={background:S.bg,border:"0.5px solid "+S.border,borderRadius:8,width:42,height:42,fontSize:22,cursor:"pointer",color:S.text,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0};
  var children=[
    RC("span",{key:"lbl",style:{fontSize:13,color:"#6b5740",fontWeight:700}},label),
    RC("div",{key:"ctrl",style:{display:"flex",alignItems:"center",gap:6}},
      RC("button",{key:"dec",style:btnS,onPointerDown:onDec},"-"),
      RC("span",{key:"val",style:{minWidth:56,textAlign:"center",fontSize:15,fontWeight:700,color:S.text}},display||String(value)),
      RC("button",{key:"inc",style:btnS,onPointerDown:onInc},"+")
    )
  ];
  if(extra) children.push(extra);
  if(note) children.push(RC("span",{key:"note",style:{fontSize:13,color:S.muted}},note));
  return RC("div",{style:{display:"flex",alignItems:"center",gap:8,marginTop:8,flexWrap:"wrap"}},children);
}
...
function ManualModal(props){
  ...
  var ls=useState(true);var locked=ls[0],setLocked=ls[1];
  if(!booking) return null;
  var s=toMins(booking.time||"13:00"),e=s+(booking.duration||90);
  var otherSlots=bookings.filter(function(b){return b&&b.id!==booking.id&&b.date===booking.date&&b.status!=="cancelled"&&(b.tables||[]).length>0;}).map(function(b){return {tables:b.tables||[],s:toMins(b.time),e:toMins(b.time)+(b.duration||90)};});
  var busy=getBusy(otherSlots,s,e);
  function toggle(id){if(busy.has(id)) return;var next=selected.includes(id)?selected.filter(function(x){return x!==id;}).concat():selected.concat([id]);var h1=next.includes("i1"),h4=next.includes("i4"),h2=next.includes("i2"),h3=next.includes("i3");if(h1&&h4&&(!h2||!h3)) return;setSelected(next);}
  var groups=[
  ...
  var cap=(function(){var k=selected.slice().sort().join("|");var c=VALID_COMBOS.find(function(x){return x.ids.slice().sort().join("|")===k;});return c?c.cap:selected.reduce(function(a,id){var t=ALL_TABLES.find(function(x){return x.id===id;});return a+(t?t.capacity:0);},0);})();
  var conflict=selected.length>=2&&!canAssign(selected,otherSlots,s,e);
  var ok=selected.length>0&&cap>=booking.size&&!conflict;
  var summaryColor=conflict?"#991b1b":ok?"#166534":"#9a3412";
  var summaryText=selected.length===0?"Select tables above.":conflict?"Conflict: cannot use these tables together.":"Capacity: "+cap+(cap>=booking.size?" (fits "+booking.size+" pax)":" — need "+booking.size+" pax");
  var clearBtn=selected.length>0?RC("button",{key:"clr",style:mkBtn({marginTop:8,fontSize:12}),onClick:function(){setSelected([]);}},"Clear"):null;
  var assignLabel="Assign";
  var groupEls=groups.map(function(grp){
    var noteEl=grp.note?RC("div",{key:"note",style:{fontSize:11,color:S.muted,marginBottom:6,fontStyle:"italic"}},grp.note):null;
    var tableEls=grp.tables.map(function(t){
      var isBusy=busy.has(t.id),isSel=selected.includes(t.id);
      var bg=isSel?grp.bg:isBusy?"#fee2e2":S.bg;
      var clr=isSel?grp.color:isBusy?"#991b1b":S.text;
      var brd="2px solid "+(isSel?grp.color:isBusy?"#fca5a5":grp.border);
      var subClr=isSel?grp.color:S.muted;
      return RC("button",{key:t.id,onClick:function(){toggle(t.id);},style:{minWidth:60,minHeight:48,padding:"8px 16px",borderRadius:8,border:brd,background:bg,color:clr,fontWeight:700,fontSize:14,cursor:isBusy?"not-allowed":"pointer",opacity:isBusy?0.55:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}},
        RC("span",null,t.id),RC("span",{style:{fontSize:10,fontWeight:500,color:subClr}},isBusy?"busy":"cap "+t.cap));
    });
  ...
  var lockBg=locked?"#fef9c3":S.bg;
  var lockBrd="0.5px solid "+(locked?"#fde68a":S.border);
  var lockTitleClr=locked?"#854d0e":S.text;
  var lockSubClr=locked?"#92400e":S.muted;
  return RC(Overlay,{onClose:onClose},
    ...
    RC("div",{style:{fontSize:13,color:S.muted,marginBottom:4}},booking.name+" · "+booking.size+" pax · "+booking.time+"–"+toTime(e)),
    RC("div",{style:{fontSize:12,color:S.muted,marginBottom:14,fontStyle:"italic"}},"Tap tables to select / deselect."),
    RC("div",null,groupEls),
    RC("div",{style:{marginTop:14,marginBottom:4,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",borderRadius:8,background:lockBg,border:lockBrd}},
      RC("div",null,RC("div",{style:{fontSize:13,fontWeight:700,color:lockTitleClr}},"Lock assignment"),RC("div",{style:{fontSize:11,color:lockSubClr,marginTop:2}},"Prevent system from reshuffling this booking")),
      RC(Toggle,{on:locked,onClick:function(){setLocked(function(l){return !l;});}})),
    RC("div",{style:{marginTop:8,padding:"12px 14px",borderRadius:8,background:S.bg,border:"0.5px solid "+(conflict?"#fca5a5":ok?"#86efac":S.border)}},
      RC("div",{style:{fontSize:14,fontWeight:700,color:S.text,marginBottom:4}},"Selected: "+(selected.length?selected.join(" + "):"none")),
      RC("div",{style:{fontSize:13,color:summaryColor,fontWeight:500}},summaryText),
      clearBtn),
    RC("div",{style:{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}},
      ...
      RC("button",{disabled:!ok,onClick:function(){if(ok)onSave(selected,locked);},style:{background:ok?S.accent:"#ccc",border:"none",borderRadius:8,padding:"10px 20px",cursor:ok?"pointer":"not-allowed",fontSize:14,fontWeight:700,color:"#fff",minHeight:44}},assignLabel)));
}
...
function TimelineView(props){
  ...
  var headerTicks=QUARTER_HOURS.concat([CLOSE*60]).map(function(m){var isH=m%60===0;var labelEl=isH?RC("span",{style:{position:"absolute",top:4,left:4,fontSize:10,fontWeight:700,color:"#7a6a52",whiteSpace:"nowrap",pointerEvents:"none"}},String(Math.floor(m/60)).padStart(2,"0")+":00"):null;return RC("div",{key:m,style:{position:"absolute",top:0,left:pct(m),bottom:0,borderLeft:isH?"1px solid #c8b99a":"0.5px solid #ddd0b8"}},labelEl);});
  var tableRows=ALL_TABLES.map(function(tbl){var id=tbl.id,indoor=isIn(id);var rows=day.filter(function(b){return (b.tables||[]).includes(id);});return RC("div",{key:id,style:{display:"flex",alignItems:"center",height:ROW_H+"px",borderBottom:"1px solid #c8b99a"}},
    RC("div",{style:{width:LABEL_W+"px",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:6}},RC("span",{style:{fontSize:11,fontWeight:700,padding:"3px 6px",borderRadius:8,background:indoor?"#ede9fe":"#fef3c7",color:indoor?"#5b21b6":"#92400e",border:"0.5px solid "+(indoor?"#c4b5fd":"#fcd34d")}},id)),
  ...
  return RC("div",{style:{background:S.card,borderRadius:12,border:"0.5px solid "+S.border,padding:"10px 12px",overflowX:"auto"}},
    RC("div",{style:{minWidth:320,width:"100%",boxSizing:"border-box"}},
      RC("div",{style:{display:"flex",alignItems:"stretch",marginBottom:2}},
        ...
        RC("div",{style:{flex:1,position:"relative",borderBottom:"1px solid #c8b99a",background:"#f0e8d0",borderRadius:"0 6px 0 0",height:24}},headerTicks)),
...
export default function App(){
  ...
  function save(){
    ...
    try{
      ...
      if(editId){
        ...
        var upd=bookings.map(function(b){if(b.id!==editId) return b;return Object.assign({},b,{name:f.name,phone:f.phone,date:f.date,time:f.time,size:size,duration:dur,preference:f.preference,notes:f.notes,status:f.status,tables:mt.length?mt:(!needsR?b.tables:[]),customDur:f.customDur||null,_manual:mt.length>0,_locked:mt.length>0?true:b._locked});});
        setBookings(applyOpt(upd,f.date));if(needsR) flash();setShowForm(false);setViewDate(f.date);
      } else {
        var newId=genId();
        var nb={id:newId,name:f.name,phone:f.phone,date:f.date,time:f.time,size:size,duration:dur,preference:f.preference,notes:f.notes,status:"confirmed",tables:mt.length?mt:[],customDur:f.customDur||null,_manual:mt.length>0,_locked:mt.length>0};
  ...
  function manualAssign(bookingId,tables,locked){setBookings(function(b){return b.map(function(x){return x.id===bookingId?Object.assign({},x,{tables:tables,_conflict:false,_manual:true,_locked:locked===true}):x;});});setManualTarget(null);}
  ...
  var tablesBtn=(function(){
    ...
    if(editId){
      ...
      return RC("div",{style:{marginTop:14,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}},
        RC("button",{style:mkBtn({border:"0.5px solid #7dd3fc",color:"#0369a1",fontWeight:700}),onClick:function(){setManualTarget(editId);}},"= Tables"),
        tbl?tbl.map(function(id){return RC(TBadge,{key:id,id:id});}):RC("span",{style:{fontSize:12,color:S.muted,fontStyle:"italic"}},"auto-assigned"),
        mt?RC("button",{style:mkBtn({fontSize:12}),onClick:function(){setForm(function(f){return Object.assign({},f,{manualTables:[]});});}},"Clear"):null);
    }
    return RC("div",{style:{marginTop:14,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}},
      RC("button",{style:mkBtn({border:"0.5px solid #7dd3fc",color:"#0369a1",fontWeight:700}),onClick:function(){setManualTarget("__new__");}},"= Tables"),
      mt?mt.map(function(id){return RC(TBadge,{key:id,id:id});}):RC("span",{style:{fontSize:12,color:S.muted,fontStyle:"italic"}},"auto-assigned"),
      mt?RC("button",{style:mkBtn({fontSize:12}),onClick:function(){setForm(function(f){return Object.assign({},f,{manualTables:[]});});}},"Clear"):null);
  })();

  var quickStatusBtns=editId?RC("div",{style:{marginTop:14,display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}},
    RC("span",{style:{fontSize:13,color:"#6b5740",fontWeight:700,marginRight:4}},"Quick status:"),
  ...
  var errorEl=error?RC("div",{style:{color:"#991b1b",fontSize:13,marginTop:10,padding:"8px 12px",background:"#fee2e2",borderRadius:8}},error):null;
  ...
  var ineffBanner=(!reshuffled&&inefficient)?RC("div",{style:{background:"#fff7ed",border:"0.5px solid #fed7aa",borderRadius:8,padding:"10px 14px",marginBottom:10,fontSize:13,fontWeight:700,color:"#9a3412",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}},RC("span",null,"Tables could be reshuffled for better efficiency."),RC("button",{onClick:forceReshuffle,style:{background:"#9a3412",color:"#fff",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:700,minHeight:36}},"Reshuffle")):null;
  ...
  var durRow=RC(StepperRow,{
    label:"Duration:",
    value:dur,
    onDec:function(e){e.preventDefault();var v=Math.max(15,Math.min(480,dur-15));setForm(function(f){return Object.assign({},f,{customDur:v===auto?null:v});});},
    onInc:function(e){e.preventDefault();var v=Math.max(15,Math.min(480,dur+15));setForm(function(f){return Object.assign({},f,{customDur:v===auto?null:v});});},
    display:dur+" min",
    extra:resetDurBtn,
    note:"End: "+endTime
  });
  ...
  var formModal=showForm?RC(Overlay,{onClose:function(){setShowForm(false);}},
    ...
    RC("div",{style:{display:"grid",gridTemplateColumns:formCols,gap:12}},
      ...
      RC(Fld,{label:"Phone number"},RC("input",{value:form.phone,onChange:function(e){setForm(function(f){return Object.assign({},f,{phone:e.target.value});});},placeholder:"+34 600 000 000",style:inp()})),
      ...
      RC(Fld,{label:"Number of guests"},RC("div",{style:{display:"flex",alignItems:"center",gap:6}},
        RC("button",{style:{background:S.bg,border:"0.5px solid "+S.border,borderRadius:8,width:42,height:42,fontSize:22,cursor:"pointer",color:S.text,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},onPointerDown:function(e){e.preventDefault();var v=Math.max(1,(Number(form.size)||2)-1);setForm(function(f){return Object.assign({},f,{size:v});});}},"-"),
        RC("span",{style:{minWidth:56,textAlign:"center",fontSize:15,fontWeight:700,color:S.text}},String(Number(form.size)||2)),
        RC("button",{style:{background:S.bg,border:"0.5px solid "+S.border,borderRadius:8,width:42,height:42,fontSize:22,cursor:"pointer",color:S.text,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},onPointerDown:function(e){e.preventDefault();var v=Math.min(25,(Number(form.size)||2)+1);setForm(function(f){return Object.assign({},f,{size:v});});}},"+")))),
    RC(Fld,{label:"Notes",style:{marginTop:12}},RC("textarea",{value:form.notes,onChange:function(e){setForm(function(f){return Object.assign({},f,{notes:e.target.value});});},rows:2,placeholder:"Allergies, special requests...",style:Object.assign({},inp(),{resize:"vertical"})})),
    RC("div",{style:{marginTop:4}},durRow),
    tablesBtn,
    quickStatusBtns,
    errorEl,
  ...
  var manualModal=manualBooking?RC(ManualModal,{booking:manualBooking,bookings:manualTarget==="__new__"?bookings.filter(function(b){return b.date===form.date;}):bookings,onSave:function(tables,locked){if(manualTarget==="__new__"){setForm(function(f){return Object.assign({},f,{manualTables:tables});});setManualTarget(null);}else{manualAssign(manualBooking.id,tables,locked);}},onClose:function(){setManualTarget(null);}}):null;

  return RC("div",{style:{background:S.bg,minHeight:"100vh",padding:isMobile?"12px":"16px",fontFamily:"var(--font-sans)",color:S.text}},
