...
| Settings › App › **Automatic**, above **Dark mode** | **automatic theme** (`theme: "auto"`, `useUserPrefs.js`) | Follows this device's light/dark setting, live. Dark mode is locked while it is on. Saved per account like the theme itself; an account that never chose reads as Automatic. |
