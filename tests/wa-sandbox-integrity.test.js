...
// GATE and the comment above a gate habitually quotes it. Two of the assertions
// here now check that an old gating shape is ABSENT, which a raw read would
// fail the moment someone explained the change in a comment beside it. That is
// this repo's recorded trap (`tests/csp.test.js`' boot block; `src/index.css`'s
// header) arriving in the one file whose whole job is to survive a merge.
...
describe("WA sandbox — edits to shared PROD files survive a sync", () => {
  ...
  it("the I and X shortcuts exist, and each is gated on the RIGHT thing", () => {
    // v18.0.0 phase 5 split these two. They were both WA_SANDBOX-gated while the
    // module could only exist in a sandbox build; now the INBOX ships to
    // production behind the module switch and only the SIMULATOR is sandbox-only.
    // The invariant this test was written for is unchanged and is the reason it
    // still checks each one separately: gated, not merely present — an ungated
    // handler is a second door to a surface whose front door is gated, and
    // gating one door is gating none.
  ...
  it("the WhatsApp settings tab is spliced into the ONE tab list", () => {
    ...
    //
    // v18.0.0 phase 5: the splice became a declaration. `WA_SANDBOX ? [...] : []`
    // removed the tab from the list at BUILD time; `module: "whatsapp"` leaves it
    // in the one list and lets `visibleTabs` hide it, which is what makes the
    // ←/→ cycle and the tab bar agree without either knowing about WhatsApp.
    const chrome = read("src/components/SettingsChrome.jsx");
    expect(chrome).toMatch(/id: "whatsapp",\s*label: "WhatsApp",\s*module: "whatsapp"/);
    expect(chrome, "the WA tab is back to a build-time splice — an admin could not switch it on")
      .not.toMatch(/WA_SANDBOX \?[^:]*id: "whatsapp"/);
  });
